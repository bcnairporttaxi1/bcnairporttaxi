...
export async function POST(req: Request) {
  ...
  const checkoutId = str(body.id) ?? str(body.checkout_id) ?? str(nested(body, 'checkout')?.id);
  const reference =
    str(body.checkout_reference) ?? str(nested(body, 'checkout')?.checkout_reference);
  ...
  const outcome = await confirmBookingPayment(booking.reference).catch((err) => {
...
function nested(o: Record<string, unknown>, k: string): Record<string, unknown> | undefined {
  const v = o[k];
